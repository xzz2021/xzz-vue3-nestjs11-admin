...
export const formatToTree = <T extends { id: number; parentId: number | null; sort: number | null }>(
  ...
  const array = deepClone(array2) //  优化方案  ???????
...
export const ensureRouteTree = <T extends { id?: string | number; parentId?: string | number | null; children?: T[] }>(
  routes: T[]
): T[] => {
  if (!routes?.length) {
    return []
  ...
  if (routes.some((route) => route.children?.length)) {
    return routes
  }

  if (routes.some((route) => route.parentId !== undefined && route.id !== undefined)) {
    return formatToTree(routes as unknown as Parameters<typeof formatToTree>[0]) as unknown as T[]
  }
