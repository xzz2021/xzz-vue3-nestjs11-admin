...
const lockedCategory = computed(() => props.category)
const isLoginOnly = computed(() => lockedCategory.value === 'login')

const actionOptions = computed(() => {
  const values = isLoginOnly.value ? LOGIN_ACTIONS : [...LOGIN_ACTIONS, ...OPERATION_ACTIONS]
...
const { tableRegister, tableState, tableMethods } = useTable<AuditLogItem, string>({
  fetchDataApi: async () => {
    ...
    const params = {
      ...
      ...unref(searchParams),
      ...(lockedCategory.value ? { category: lockedCategory.value } : {})
    }
...
const searchParams = ref<Recordable>(lockedCategory.value ? { category: lockedCategory.value } : {})
...
const searchSchema = computed<FormSchema[]>(() => {
  ...
  if (!lockedCategory.value) {
    schema.push({
      field: 'category',
      label: t('auditLog.category'),
      component: 'Select',
      componentProps: {
        options: [
          { label: t('auditLog.categoryLogin'), value: 'login' },
          { label: t('auditLog.categoryOperation'), value: 'operation' }
        ],
        clearable: true
      }
    })
