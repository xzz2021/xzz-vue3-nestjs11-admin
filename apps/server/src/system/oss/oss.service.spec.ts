...
describe('OssService', () => {
  ...
  beforeEach(() => {
    send = vi.fn();
    (presign as Mock).mockResolvedValue('https://s3.example/presigned');
    service = new OssService({ send } as any, configured());
  });
  ...
  it('creates a folder placeholder and rejects existing names', async () => {
    ...
    const put = send.mock.calls.find(
      (call: unknown[]) => call[0] instanceof PutObjectCommand,
  ...
  it('copies then deletes a single file', async () => {
    ...
    expect(
      send.mock.calls.some(
        (call: unknown[]) => call[0] instanceof CopyObjectCommand,
      ),
    ).toBe(true);
    expect(
      send.mock.calls.some(
        (call: unknown[]) => call[0] instanceof DeleteObjectCommand,
      ),
    ).toBe(true);
  });
